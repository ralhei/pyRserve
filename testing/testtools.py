...
def start_pyRserve():
    RPORT = 6311
    # Start Rserve
    rProc = subprocess.Popen(['R', 'CMD', 'Rserve.dbg', '--no-save'], stdout=open('/dev/null'), stderr=subprocess.PIPE)
    # wait a moment until Rserve starts listening on RPORT
    time.sleep(0.6)
    if rProc.poll():
        # process has already terminated, so provide its output on stderr to the user:
        raise RuntimeError('Rserve has terminated prematurely with the following message:  %s' % rProc.stderr.read())

    # store original socket timeout and set timeout to new value during startup of Rserve:
    defaultTimeout = socket.getdefaulttimeout()
    socket.setdefaulttimeout(1)

    rserv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    cnt = 0
    # give it a maximum of 10 tries with some sleep in between to wait for Rserve to come into action!
    while cnt < 10:
        try:
        ...
        # after trying 10 times we still got no connection to Rserv - something must be wrong.
        raise RuntimeError('Could not connect to Rserv over the network')

    # set back original default timeout value:
    socket.setdefaulttimeout(defaultTimeout)

    # make a simple test that Rserve really answers correctly by looking at the first few bytes:
    hdr = rserv.recv(1024)
    rserv.close()
    if not hdr.startswith(b'Rsrv01'):
        rProc.terminate()
        raise RuntimeError('received wrong header information from socket (was: "%s")' %hdr[:10])
    return rProc
